...
function cvConfig(): {
  ...
  const accessHeaders: Record<string, string> =
    e.CF_ACCESS_CLIENT_ID && e.CF_ACCESS_CLIENT_SECRET
      ? {
          'CF-Access-Client-Id': e.CF_ACCESS_CLIENT_ID,
...
// eslint-disable-next-line sonarjs/cognitive-complexity -- grandfathered at 21; the flat tool-dispatch table
export async function api(
  ...
  try {
    ...
    throw new Error(
      `Could not reach cv-editor at ${base}: ${e.message}. Make sure the cv-editor server is running.`,
    );
  }
...
const idList = {
  ...
  minItems: 1,
  description: 'Ids in the desired order',
};
...
const toolDefs: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      'Update personal info fields (firstName, lastName, position, email, github, …). Only passed fields change; all values must be strings.',
    inputSchema: {
      type: 'object',
      properties: {
        person_id: personId,
  ...
  },
  {
    ...
    description:
      "Update a section's title (and optionally slug/type). Pass at least one of slug, type, title.",
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: (a) => {
      ...
      if (!Object.keys(body).length)
        throw new Error('cv_update_section needs at least one of: slug, type, title');
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { entry_id: { type: 'integer' }, fields: { type: 'object' } },
  ...
  },
  {
    ...
    handler: (a) =>
      api('POST', `/api/entries/${enc(a.entry_id)}/items`, {
        content: a.content,
  ...
  },
  {
    ...
    handler: (a) => {
      ...
      if (!Object.keys(body).length)
        throw new Error('cv_update_bullet needs at least one of: content, title');
  ...
  // ---- Tags ----
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        target: { type: 'string', enum: ['entry', 'bullet'] },
        id: { type: 'integer' },
        tags: tagList,
      },
    ...
    handler: (a) =>
      api('POST', `/api/${a.target === 'bullet' ? 'items' : 'entries'}/${enc(a.id)}/tags`, {
        tags: a.tags,
      }),
  },
  {
    ...
    handler: (a) =>
      api(
        'DELETE',
        `/api/${a.target === 'bullet' ? 'items' : 'entries'}/${enc(a.id)}/tags/${enc(a.tag)}`,
      ),
  },
  {
    name: 'cv_search_tags',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        limit: {
          ...
          minimum: 1,
          maximum: 50,
          description: 'Max suggestions per item (default 5)',
        },
        min_score: {
          type: 'number',
          minimum: 0,
  ...
  },
  {
    ...
    handler: (a) =>
      api('PUT', `/api/variants/${enc(a.variant_id)}/rules`, {
        include: a.include || [],
        exclude: a.exclude || [],
      }),
  },
  ...
  },
  {
    ...
    handler: (a) =>
      api('PUT', `/api/variants/${enc(a.variant_id)}/sections`, { sections: a.sections }),
  },
  {
    ...
    handler: (a) =>
      api('PUT', `/api/variants/${enc(a.variant_id)}/overrides`, {
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { variant_id: variantId, title: { type: 'string' }, body: { type: 'string' } },
      required: ['variant_id', 'title', 'body'],
      additionalProperties: false,
    },
    handler: (a) =>
      api('POST', `/api/variants/${enc(a.variant_id)}/letter-sections`, {
        title: a.title,
        body: a.body,
      }),
  },
  ...
  },
  {
    ...
    handler: async (a) => {
      ...
      if (!secret)
        throw new Error(
          'Server misconfigured: no signing secret (COOKIE_SECRET/GOOGLE_CLIENT_SECRET) for PDF links.',
        );
      ...
      return {
        __content: [
          {
            type: 'text',
            text: `PDF for variant ${a.variant_id} is ready — download (link valid ~5 min, compiles on open): ${base}/pdf/${token}`,
          },
        ],
      };
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        layout_id: {
          type: ['string', 'null'],
          description: 'Layout id, or null to revert to the default',
        },
      },
  ...
  },
  {
    name: 'cv_get_layout',
    description:
      "Get one layout's full detail + last verification report: {id,name,version,kinds,status,source,manifest,report}. (cv_list_layouts returns the summary list.)",
    inputSchema: {
      type: 'object',
      properties: { layout_id: layoutId },
      required: ['layout_id'],
  ...
  },
  {
    ...
    description:
      "Reorder entries within a section. ids = the section's entry ids in the desired order.",
    inputSchema: {
      type: 'object',
      properties: { section_id: { type: 'integer' }, ids: idList },
      required: ['section_id', 'ids'],
      additionalProperties: false,
    },
    handler: (a) =>
      api('PATCH', `/api/sections/${enc(a.section_id)}/entries/order`, { ids: a.ids }),
  },
  {
    name: 'cv_reorder_bullets',
    description:
      "Reorder bullets within an entry. ids = the entry's bullet/item ids in the desired order.",
    inputSchema: {
      type: 'object',
      properties: { entry_id: { type: 'integer' }, ids: idList },
      required: ['entry_id', 'ids'],
      additionalProperties: false,
    },
  ...
  // ---- Variant rename + cover-letter paragraph edit/delete/reorder ----
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { variant_id: variantId, name: { type: 'string', minLength: 1 } },
  ...
  },
  {
    ...
    description:
      'Edit a cover-letter paragraph (title and/or body). letter_section_id from cv_get_variant (letterSections).',
    inputSchema: {
      type: 'object',
      properties: {
        variant_id: variantId,
        letter_section_id: { type: 'integer' },
        title: { type: 'string' },
        body: { type: 'string' },
      },
      required: ['variant_id', 'letter_section_id'],
      additionalProperties: false,
    },
    handler: (a) => {
      const body: Record<string, any> = {};
      ...
      return api(
        'PUT',
        `/api/variants/${enc(a.variant_id)}/letter-sections/${enc(a.letter_section_id)}`,
        body,
      );
    },
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { variant_id: variantId, letter_section_id: { type: 'integer' } },
      required: ['variant_id', 'letter_section_id'],
      additionalProperties: false,
    },
    handler: (a) =>
  ...
  },
  {
    ...
    description:
      "Reorder a cover letter's paragraphs. ids = letter-section ids in the desired order.",
    inputSchema: {
      type: 'object',
      properties: { variant_id: variantId, ids: idList },
      required: ['variant_id', 'ids'],
      additionalProperties: false,
    },
    handler: (a) =>
  ...
  // ---- LinkedIn / Indeed / Handshake export + drift tracking ----
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        format: {
          type: 'string',
          enum: ['linkedin', 'plaintext', 'markdown'],
...
function formatErrors(
  errors: Array<{ instanceLocation?: string; keyword?: string; error?: string }>,
): string {
  return errors
    .map((e) => `${e.instanceLocation || '/'} ${e.error || e.keyword || 'invalid'}`.trim())
    .join('; ');
}
...
export async function callTool(name: string, args: any): Promise<any> {
  ...
  if (!result.valid)
    throw new Error(`Invalid arguments for ${name}: ${formatErrors(result.errors as any)}`);
