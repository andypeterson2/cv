...
module.exports = function createLayoutsRouter(getDb, projectRoot) {
  ...
  router.get('/default', wrap((req, res) => {
    res.json({ layout_id: getDb().getDefaultLayoutId() });
  }));

  router.put('/default', wrap((req, res) => {
  ...
  router.post('/', uploadRateLimit, upload.single('bundle'), wrap(async (req, res) => {
    ...
    try {
      ...
      if (!root) throw new AppError('Zip must contain a layout.json (at its root or in a single top-level folder)', 422);

      let manifest;
      try { ({ manifest } = loadLayout(root)); }
      catch (e) { throw new AppError('Invalid bundle: ' + e.message, 422); }

      const existing = getDb().getLayout(manifest.id);
      if (existing && existing.source === 'builtin') {
        throw new AppError(`"${manifest.id}" is the id of a builtin layout and cannot be overwritten`, 409);
      }

      const report = await verifyLayout(root, { assetsDir: ASSETS_DIR, samples: gatherSamples(getDb()) });
      if (!report.ok) {
        return res.status(422).json({ error: { code: 'verification_failed', message: 'Layout failed verification', details: report } });
      }
      ...
      const row = upsertFromManifest(getDb(), manifest, {
        status: 'active', source: 'upload', checksum: bundleChecksum(dest), report,
      });
  ...
  router.post('/:id/verify', wrap(async (req, res) => {
    const layout = getDb().getLayout(req.params.id);
    if (!layout) throw new NotFoundError('Layout not found');
    const report = await verifyLayout(layoutDirForRow(layout), { assetsDir: ASSETS_DIR, samples: gatherSamples(getDb()) });
    upsertFromManifest(getDb(), layout.manifest || { id: layout.id, name: layout.name, version: layout.version, engine: layout.engine, kinds: layout.kinds }, {
      status: report.ok ? 'active' : 'invalid', source: layout.source, checksum: layout.checksum, report,
    });
