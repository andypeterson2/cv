...
async function buildMain() {
  ...
  const e1 = (
    await request('POST', `/api/sections/${exp}/entries`, { fields: { position: 'Engineer' } })
  ).body.id;
  const i1 = (await request('POST', `/api/entries/${e1}/items`, { content: 'Built frontend' })).body
    .id;
  const e2 = (
    await request('POST', `/api/sections/${exp}/entries`, { fields: { position: 'Intern' } })
  ).body.id;
...
describe('Sections / entries / items', () => {
  test('CRUD and ordering', async () => {
    ...
    expect((await request('GET', `/api/sections/${exp}`)).body.entries.map((e) => e.id)).toEqual([
      e2,
      e1,
    ]);
    ...
    expect((await request('GET', `/api/entries/${e1}`)).body.fields.position).toBe(
      'Senior Engineer',
    );
...
describe('Fuzzy tags', () => {
  test('normalization folds case/separator variants and de-dupes', async () => {
    ...
    await request('POST', `/api/entries/${e1}/tags`, {
      tags: ['Front End', 'front_end', 'Machine Learning'],
    });
    expect((await request('GET', `/api/entries/${e1}`)).body.tags).toEqual([
      'front-end',
      'machine-learning',
    ]);
  ...
  test('rule expansion materializes near-miss tags; resolution stays exact', async () => {
    ...
    expect(
      resolved.sections.find((s) => s.id === 'experience').entries.map((e) => e.fields.position),
    ).toEqual(['Engineer']);
    ...
    expect((await request('GET', `/api/variants/${v}`)).body.rules.include.sort()).toEqual([
      'front-end',
      'frontend',
    ]);
    ...
    expect(
      resolved.sections
        ...
        .entries.map((e) => e.fields.position)
        .sort(),
...
describe('Variants', () => {
  ...
  test('override forces inclusion against tags', async () => {
    ...
    const resolved = (await request('GET', `/api/variants/${v}/resolve`)).body;
    const positions = resolved.sections
      .find((s) => s.id === 'experience')
      .entries.map((e) => e.fields.position)
      .sort();
  ...
  test('coverletter variant: per-variant header via PATCH /header, GET, resolve', async () => {
    ...
    expect(
      (
        await request('PATCH', `/api/variants/${v}/header`, {
          recipientName: 'Acme',
          opening: 'Dear Acme,',
        })
      ).status,
    ...
    expect((await request('GET', `/api/variants/${v}`)).body.header).toMatchObject({
      recipientName: 'Acme',
      opening: 'Dear Acme,',
    });
    expect(
...
describe('Catalog + health', () => {
  test('catalog returns section types; health states liveness and nothing else', async () => {
    ...
    // The cross-repo /health contract's required keys.
